...
import rospy
import actionlib
from demo.msg import robotAction, robotFeedback, robotResult
import math
import tf
import PyKDL 
import tf2_ros
...
class action_server():
    ...
    def __init__(self):
        ...
        self.a_server = actionlib.SimpleActionServer("arm_as", robotAction, execute_cb = self.execute_cb, auto_start = False)
        self.a_server.start()
    ...
    def execute_cb(self, goal):
        ...
        goal_x_lin = round(goal.coord[0],1)
        goal_y_lin = round(goal.coord[1],1)
        goal_z_lin = round(goal.coord[2],1)
        goal_x_ang = round(goal.coord[3],1)
        goal_y_ang = round(goal.coord[4],1)
        goal_z_ang = round(goal.coord[5],1)
        goal_w_ang = round(goal.coord[6],1)
        ...
        while ((completion_status_lin == False) or (completion_status_ang == False)):
            ...
            if ((round(diff_x,3) != 0.000) or (round(diff_y,3) != 0.000) or (round(diff_z,3) != 0.000)):
                vel_msg.linear.x = round((round(diff_x,3)/d_t), 2)
                vel_msg.linear.y = round((round(diff_y,3)/d_t), 2)
                vel_msg.linear.z = round((round(diff_z,3)/d_t), 2)
        ...
        if (goal.type == 'PICK'):
            ...
            joint.position = list(joint_position)
            #print(list(joint_position))
            joint.position[-1] = 0.0
            joint.position[-2] = 0.0
            #print(joint.position)
            #joint.position.insert(7,0.0)
            #joint.position.insert(8,0.0)
            #print(joint.position[7], joint.position[8])
            viz_pub.publish(joint)
            time.sleep(10)
            ...
            joint.position = list(joint_position)
            joint.position.insert(7,0.035) 
            joint.position.insert(8,0.035)
            viz_pub.publish(joint)
            time.sleep(10)
        ...
        if (completion_status_lin and completion_status_ang):
            if success:
                ...
                result.final_coord = [ee2b.transform.translation.x, ee2b.transform.translation.y, ee2b.transform.translation.z, ee2b.transform.rotation.x, ee2b.transform.rotation.y, ee2b.transform.rotation.z, ee2b.transform.rotation.w]
...
if __name__=="__main__":
    ...
    viz_sub = rospy.Subscriber("/viz/joint_states", JointState, get_state)
